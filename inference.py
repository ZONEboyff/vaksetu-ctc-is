...
from config import MODEL_PATH, VOCAB, BLANK_IDX, MIN_FRAMES_PER_SAMPLE, BLANK_RATIO_THRESHOLD
...
# ── Skeleton connections (same as collect_data.py) ────────────────────────
HAND_CONNECTIONS = [
    (0, 1), (1, 2), (2, 3), (3, 4),
    (0, 5), (5, 6), (6, 7), (7, 8),
...
def draw_landmarks_on_frame(frame, pose_result, hand_result, face_result=None):
    ...
    # Face key points (cyan dots — chin, jaw, ears, nose, forehead, mouth)
...
def ctc_greedy_decode(log_probs_np, blank=BLANK_IDX):
    """
    Greedy CTC decode with blank-ratio gate.
    log_probs_np : (T, C)  — numpy array (single sequence)
    Returns list of gloss strings, or [] if mostly blank.
    """
    indices   = log_probs_np.argmax(axis=-1)   # (T,)

    # Blank-ratio gate: if too many blank frames, treat as no sign
...
def load_model(device="cpu"):
    ...
    model.eval()
    print(f"[Model] Loaded '{MODEL_PATH}'  epoch={ckpt['epoch']}  val_loss={ckpt['val_loss']:.4f}")
    return model


# ─────────────────────────────────────────────────────────────────────────────
# Inference on a numpy sequence
# ─────────────────────────────────────────────────────────────────────────────

def predict(model, sequence_np, device="cpu"):
    """sequence_np: (T, 288). Returns list of gloss strings."""
    t = torch.from_numpy(sequence_np).float().unsqueeze(1).to(device)  # (T, 1, 288)
    with torch.no_grad():
...
# ─────────────────────────────────────────────────────────────────────────────
# Shared: build Tasks API landmarkers
# ─────────────────────────────────────────────────────────────────────────────
...
def infer_from_file(video_path, model, device):
    ...
    return predict(model, np.array(frames, dtype=np.float32), device)
...
def overlay_text(frame, lines, start_y=30, color=(255, 255, 255)):
    for i, line in enumerate(lines):
        y = start_y + i * 30
        cv2.putText(frame, line, (11, y + 1), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0),  2, cv2.LINE_AA)
...
def infer_from_webcam(model, device):
    ...
    if cap is None:
        print("[ERROR] Could not open webcam with any backend/index.")
        print("        Make sure no other app (Zoom, Teams, OBS …) is using the camera.")
        sys.exit(1)
    ...
    while cap.isOpened():
        ...
        overlay_text(frame, [status, f"Prediction: {result_str}", "R=Record  S=Decode  Q=Quit"], color=color)
        ...
        if key in (ord("r"), ord("R")):
            ...
            if recording:
                ...
                if len(buffer) >= MIN_FRAMES_PER_SAMPLE:
                    glosses     = predict(model, np.array(buffer, dtype=np.float32), device)
...
def main():
    parser = argparse.ArgumentParser(description="LSTM + CTC Sign Language Inference")
